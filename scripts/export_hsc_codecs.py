...
def convert_codec(name: str, legacy_codec: Any) -> tuple[ScalarCodec | LogScalarCodec | None, str]:
    """
    ...
    if codec_class != 'ScalarIdentityCodec':
        return None, f"Codec is not ScalarIdentityCodec (got {codec_class}), skipping..."
...
def test_codec_conversion(name: str, new_codec: ScalarCodec | LogScalarCodec, legacy_codec: Any) -> bool:
    ...
    try:
        ...
        if not all(abs(a - b) < 1e-5 for a, b in zip(ref_decoded_values, decoded_values)):
...
def upload_codec_to_hub(name: str, codec: ScalarCodec | LogScalarCodec, skip_upload: bool = False) -> bool:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create repo ID
            repo_id = f"polymathic-ai/aion-scalar-{name.lower().replace('_', '-')}-codec"
...
def test_uploaded_codec(name: str, legacy_codec: Any, data_dir: Path) -> bool:
    ...
    try:
        ...
        with torch.no_grad():
            output = downloaded_codec.encode(downloaded_codec.modality(value=input_batch))
        ...
        if not torch.allclose(decoded_output.value[mask], reference_decoded_batch[mask], atol=1e-5):
        ...
        torch.save(reference_encoded_batch[mask], data_dir / f"{name}_codec_encoded_batch.pt")
        torch.save(reference_decoded_batch[mask], data_dir / f"{name}_codec_decoded_batch.pt")
